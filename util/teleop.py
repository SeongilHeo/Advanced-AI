...
SUPPORTED_RENDER_MODES = {"rgb_array", "rgb_array_list"}
...
class PlayableGame:
    ...
    def __init__(
        ...
        """
        if env.render_mode not in SUPPORTED_RENDER_MODES:
            logger.error(
                "PlayableGame wrapper works only with rgb_array-like render modes, "
...
def play(
    ...
    Example::
        ...
        >>> import gymnasium as gym
        >>> from util.teleop import play
        >>> play(gym.make("CarRacing-v3", render_mode="rgb_array"), keys_to_action={
        ...                                                "w": np.array([0, 0.7, 0]),
        ...                                                "a": np.array([-1, 0, 0]),
        ...                                                "s": np.array([0, 0, 1]),
        ...                                                "d": np.array([1, 0, 0]),
    ...
    Args:
        ...
        callback: If a callback is provided, it will be executed after every step. It takes the following input:
                ...
                truncated: whether the episode ended due to a truncation condition (e.g. time limit)
                info: debug info
    ...
    if env.render_mode not in SUPPORTED_RENDER_MODES:
        logger.error(
            "play method works only with rgb_array-like render modes, "
            f"but your environment render_mode = {env.render_mode}."
        )
    ...
    while game.running:
        if terminated or truncated:
            terminated, truncated = False, False
            print(f"[Info] (teleop) total reward: {total_reward}")
            total_reward = 0.0
            obs, _ = env.reset(seed=seed)
        else:
...
class PlayPlot:
    ...
    def callback(
        ...
        Args:
            ...
            action: The action
            rew: The reward
            terminated: If the environment reached a terminal state
            truncated: If the episode ended due to truncation
